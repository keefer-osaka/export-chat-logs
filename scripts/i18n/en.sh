...
SUMMARY_FORMAT="📝 Format: Markdown (tool calls and technical details omitted)"
MSG_DONE="✅ Done! Claude Code: %CC_SESSIONS% sessions, zip: %ZIP_SIZE%, sent to Telegram"

# save-token.sh
ERR_TOKEN_REQUIRED="❌ Token required: bash save-token.sh <token> <chat_id> [timezone_offset] [lang]"
ERR_CHATID_REQUIRED="❌ Chat ID required: bash save-token.sh <token> <chat_id> [timezone_offset] [lang]"
MSG_CONFIG_SAVED="✅ Configuration saved (Token + Chat ID + Timezone %TZ_LABEL% + Language %LANG%)"
